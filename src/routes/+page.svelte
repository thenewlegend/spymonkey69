<script>
	...
	import { notifyAction, tooltipAction } from 'svelte-legos';
	...
	async function displayData() {
		...
		if (myMonkeFormatted !== '' && opMonkeFormatted !== '') {
			try {
				...
				console.log('Fetched Monke Data');
...
<div class="container">
	<div class="top">
		<div class="top-item op-left">
			...
			<span>{banana11}</span><br />
			<span>Stash : {stash11}</span><br />
		</div>

		<div class="buttons top-b">
			<button on:click={displayData}>REKT</button>
		...
		<div class="top-item top-right">
			...
			<span>{banana22}</span><br />
			<span>Stash : {stash22}</span><br />
		</div>
	</div>
...
<div class="notifications">
	<button
		...
		class="notifications"
		use:notifyAction={{
			title: 'Check Your Monke Name',
			description: 'Your monke name is wrong. Double check dummy.',
			type: 'error',
			duration: 3000
		}}
	/>
	...
	<button
		...
		class="notifications"
		use:notifyAction={{
			title: 'Check Opponent Monke Name',
			description: 'Opponent monke name is wrong. Double check dummy.',
			type: 'error',
			duration: 3000
		}}
	/>
	...
	<button
		...
		class="notifications"
		use:notifyAction={{
			title: 'Invalid EOS Account',
			description: 'Not even an EOS account. Jeez !',
			type: 'error',
			duration: 3000
		}}
	/>
	...
	<button
		...
		use:notifyAction={{
			...
			description: 'Put on your specs and check monke names again ser !',
			type: 'error',
			duration: 3000
		}}
	/>
